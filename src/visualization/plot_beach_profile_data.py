...
def linear_regression(ys_orig, xs_orig):
    ...
    tmpx = np.linspace(
        np.min(xs_orig) - 0.05 * tmprange, np.max(xs_orig) + 0.05 * tmprange, 1000
    )  # 5% buffer on new x vector
...
def smooth_profile(profiles):
    ...
    for ii in range(len(profiles[:, 1])):
        ...
        smooth_profile = lowess.lowess(
            np.arange(len(profile)), profile, f=f0, iter=iters
        )
...
def lin_fit_slope(z_line):
    ...
    lindiff_profiles = np.zeros(
        np.shape(profiles[:maxreal, :])
    )  # [~np.isnan(profiles)]
...
def main():
    ...
    fig006, ax006 = plt.subplots(
        3, 3, figsize=(9, 9), num="spatial correlation, tide-tide"
    )
    ...
    for ii in tide_range:
        ...
        gsizefn = os.path.join(
            drivechar,
            "data",
            "processed",
            "grainsize",
            "beach_surveys_reprocessed_x08",
            tide,
            grid_spec + ".json",
        )
        ...
        mgs = np.pad(
            mgs0, (0, npts - len(mgs0)), "constant", constant_values=(np.nan, np.nan)
        )
        sort = np.pad(
            sort0, (0, npts - len(sort0)), "constant", constant_values=(np.nan, np.nan)
        )
        ...
        x = np.pad(
            x0, (0, npts - len(x0)), "constant", constant_values=(np.nan, np.nan)
        )
        y = np.pad(
            y0, (0, npts - len(y0)), "constant", constant_values=(np.nan, np.nan)
        )
        z = np.pad(
            z0, (0, npts - len(z0)), "constant", constant_values=(np.nan, np.nan)
        )
        ...
        if counter == 0:
            ...
            net_dz.append(np.nansum(dz) * 3.0)
            ...
            rtmp_dz_mgs, ptmp_dz_mgs, lotmp_dz_mgs, hitmp_dz_mgs = pearsonr_ci(
                tmp_dz[Ijnk:], tmp_mgs[Ijnk : len(tmp_dz)], alpha=0.05
            )
            rtmp_dz_dmgs, ptmp_dz_dmgs, lotmp_dz_dmgs, hitmp_dz_dmgs = pearsonr_ci(
                tmp_dz[Ijnk:], tmp_dmgs[Ijnk : len(tmp_dz)], alpha=0.05
            )
            rtmp_dz_sort, ptmp_dz_sort, lotmp_dz_sort, hitmp_dz_sort = pearsonr_ci(
                tmp_dz[Ijnk:], tmp_sort[Ijnk : len(tmp_dz)], alpha=0.05
            )
            rtmp_dz_dsort, ptmp_dz_dsort, lotmp_dz_dsort, hitmp_dz_dsort = pearsonr_ci(
                tmp_dz[Ijnk:], tmp_dsort[Ijnk : len(tmp_dz)], alpha=0.05
            )
            (
                rtmp_dz_last_mgs,
                ptmp_dz_last_mgs,
                lotmp_dz_last_mgs,
                hitmp_dz_last_mgs,
            ) = pearsonr_ci(tmp_dz[Ijnk:], tmp_last_mgs[Ijnk : len(tmp_dz)], alpha=0.05)
            (
                rtmp_dz_last_sort,
                ptmp_dz_last_sort,
                lotmp_dz_last_sort,
                ...
                tmp_dz[Ijnk:], tmp_last_sort[Ijnk : len(tmp_dz)], alpha=0.05
            )
            (
                rtmp_dz_last_dz,
                ptmp_dz_last_dz,
                lotmp_dz_last_dz,
                hitmp_dz_last_dz,
            ) = pearsonr_ci(tmp_dz[Ijnk:], tmp_last_dz[Ijnk : len(tmp_dz)], alpha=0.05)
    ...
    ax1_0 = ax1[0].imshow(
        ...
        vmin=-0.35,
        vmax=0.35,
        extent=[tide_range[1], tide_range[-1], 15, -30],
        aspect="auto",
    )
    ...
    if grid_spec == "cross_shore":
        ...
        fig010, ax010 = plt.subplots(
            1, 1, figsize=(3.5, 4.5), num="mean mgs, cross-shore"
        )
        ax010.plot(np.mean(smoothed_M0, 1), np.linspace(-27, 9, 13), "ko")
        ax010.plot(
            [
                np.mean(smoothed_M0, 1) - np.std(smoothed_M0, 1),
                np.mean(smoothed_M0, 1) + np.std(smoothed_M0, 1),
            ],
            [np.linspace(-27, 9, 13), np.linspace(-27, 9, 13)],
            "k-",
        )
        ax010.plot(
            [np.min(smoothed_M0, 1), np.max(smoothed_M0, 1)],
            [np.linspace(-27, 9, 13), np.linspace(-27, 9, 13)],
            "k|",
        )
        ...
        Xtmp = np.linspace(
            tide_range[1], tide_range[-1], tide_range[-1] - tide_range[1] + 1
        )
        ...
        jjj = matplotlib.colors.Normalize(
            vmin=-np.abs(np.max(smoothed_dz)), vmax=np.abs(np.max(smoothed_dz))
        )
        ...
        surf = acx1.plot_surface(
            X, Y, Z, facecolors=clrs1, linewidth=0, antialiased=False
        )
        ...
        acx1.invert_xaxis()
        acx1.view_init(elev=38.0, azim=50)
        ...
        surf = acx2.plot_surface(
            X, Y, Z, facecolors=clrs2, linewidth=0, antialiased=False
        )
        acx2.invert_xaxis()
        acx2.view_init(elev=38.0, azim=50)
    ...
    fig3, ax3 = plt.subplots(
        nrows=3, ncols=1, figsize=(3, 6), num="tide-tide change change"
    )
    ax30 = ax3[0].imshow(
        dz_line[:maxreal, :],
        cmap="bwr",
    ...
    fig004, ax004 = plt.subplots(
        2, 1, figsize=(9, 9), num="temporal correlation (lower beach), tide-tide"
    )
    ...
    pearsonr_dz_last_dz, alpha_dz_last_dz = pearsonr(
        all_dz[len(all_dz) - len(all_last_dz) :], all_last_dz
    )

    rr_dz_mgs, pp_dz_mgs, lo_dz_mgs, hi_dz_mgs = pearsonr_ci(
        all_dz, all_mgs, alpha=0.05
    )
    rr_dz_dmgs, pp_dz_dmgs, lo_dz_dmgs, hi_dz_dmgs = pearsonr_ci(
        all_dz, all_dmgs, alpha=0.05
    )
    rr_dz_last_mgs, pp_dz_last_mgs, lo_dz_last_mgs, hi_dz_last_mgs = pearsonr_ci(
        all_dz, all_last_mgs, alpha=0.05
    )
    rr_dz_last_dz, pp_dz_last_dz, lo_dz_last_dz, hi_dz_last_dz = pearsonr_ci(
        all_dz[len(all_dz) - len(all_last_dz) :], all_last_dz, alpha=0.05
    )
    ...
    plt.legend(
        [r"$\Delta z,M_0, r^2=$" + str(r1), r"$\Delta z,\Delta M_0, r^2=$" + str(r2)]
    )
    ...
    ax9[1].plot(
        [mean_cc_dz_mgs - std_cc_dz_mgs, mean_cc_dz_mgs + std_cc_dz_mgs], [0, 0], "C0-"
    )
    ax9[1].plot(mean_cc_dz_sort, 1, "C1.")
    ax9[1].plot(
        [mean_cc_dz_sort - std_cc_dz_sort, mean_cc_dz_sort + std_cc_dz_sort],
        [1, 1],
        "C1-",
    )
    ax9[1].plot(mean_cc_dz_dmgs, 2, "C2.")
    ax9[1].plot(
        [mean_cc_dz_dmgs - std_cc_dz_dmgs, mean_cc_dz_dmgs + std_cc_dz_dmgs],
        [2, 2],
        "C2-",
    )
    ax9[1].plot(mean_cc_dz_dsort, 3, "C3.")
    ax9[1].plot(
        [mean_cc_dz_dsort - std_cc_dz_dsort, mean_cc_dz_dsort + std_cc_dz_dsort],
        [3, 3],
        "C3-",
    )
    ...
    ax9[1].plot(
        [
            mean_cc_dz_last_mgs - std_cc_dz_last_mgs,
            mean_cc_dz_last_mgs + std_cc_dz_last_mgs,
        ],
        [4, 4],
        "C4-",
    ...
    ax9[1].plot(
        [
            mean_cc_dz_last_sort - std_cc_dz_last_sort,
            mean_cc_dz_last_sort + std_cc_dz_last_sort,
        ],
        [5, 5],
        "C5-",
    ...
    ax9[1].plot(
        [
            mean_cc_dz_last_dz - std_cc_dz_last_dz,
            mean_cc_dz_last_dz + std_cc_dz_last_dz,
        ],
        [6, 6],
        "C6-",
    ...
    fig11, ax11 = plt.subplots(
        2, 1, figsize=(5, 3), sharex=True, num="Hs and grainsize change"
    )
    ...
    ax11[1].plot(
        [tide_axis, tide_axis],
        [
            np.nanmean(mgs_line, axis=0) - np.nanstd(mgs_line, axis=0),
            np.nanmean(mgs_line, axis=0) + np.nanstd(mgs_line, axis=0),
        ],
        "k-",
    )
    ...
    fig12, ax12 = plt.subplots(
        3, 1, figsize=(5, 9), num="correlation against high tide elevation"
    )
